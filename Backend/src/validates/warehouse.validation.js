...
const checkWarehouse = [
    ...
    body('faxNumber').notEmpty().withMessage('Số điện thoại bắt buộc').bail().isMobilePhone('vi-VN').withMessage('Số điện không hợp lệ'),
    body('address').notEmpty().withMessage('Địa chỉ bắt buộc').bail().isString('Địa chỉ phải là chuỗi'),
    body('status').notEmpty().withMessage('Trạng thái kho bắt buộc').bail().custom((value, {req}) => {
